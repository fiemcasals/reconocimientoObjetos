...
def generate_frames():
    ...
    while True:
        ...
        # Tomar la mitad izquierda
        left_half = frame[:, :width // 2]

        # Escalarla para que ocupe todo el ancho
        frame = cv2.resize(left_half, (width, height))  # ahora ocupa todo el espacio visible

        # Actualizar dimensiones luego del resize
        height, width, _ = frame.shape

        # Detección usando el recorte original (mejor calidad)
        frame_for_net = cv2.resize(left_half, (300, 300))
        blob = cv2.dnn.blobFromImage(frame_for_net, 0.007843, (300, 300), (127.5, 127.5, 127.5))
        net.setInput(blob)
        detections = net.forward()

        # Dibujar sobre el frame reescalado
